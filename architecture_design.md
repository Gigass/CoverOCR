...
| **排版属性识别** | **Custom Multi-task Network / Regression Model** | **已实现**：<br>1. **字体分类**：微调 ResNet18 (86.26% 准确率)<br>2. **字号回归**：RandomForest + Anchor-based Features (MAE 0.03pt)<br>3. **字号名称映射**：基于回归结果吸附到标准字号 (98.08% 准确率) |
| **数据预处理** | **Scikit-learn / Pandas / NumPy** | **已实现**：<br>1. **特征工程**：锚点归一化、文本特征、几何特征<br>2. **数据增强**：旋转、模糊、噪点、亮度变化<br>3. **模板匹配**：精确文本匹配构建训练集 |
...
### 7.1 字体识别（ResNet18）
- **训练数据**: 364 样本（52 张图 × 7 个文本区域，数据增强后）
- **训练轮次**: 20 epochs
- **最终准确率**: 86.26%
- **模型大小**: ~45MB
- **推理速度**: <50ms/样本

### 7.2 字号识别（RandomForest）
- **训练数据**: 364 样本（精确匹配模板）
- **核心特征**: 
  - `height_ratio_to_anchor` (最重要，权重 57.4%)
  - `bbox_width` (权重 34.5%)
  - `text_length`, `is_chinese` 等
- **测试集性能**:
  - **Point Size MAE**: 0.08 pt (训练集 0.03 pt)
  - **R² Score**: 0.992
